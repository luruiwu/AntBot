...
class OutAndBack():
    def __init__(self):
        ...
        self.move_angle(radians(179)     
        rospy.sleep(0.5)
        
        #for i in range(2):
        self.move_distance(goal_distance[2], direction_x[2], direction_y[2])
        self.move_distance(goal_distance[1] - 0.4, direction_x[1], direction_y[1])
        self.move_distance(goal_distance[0] + 0.1, direction_x[0], direction_y[0])
        #print "goal_distance get"
        
        self.move_angle(radians(178))  
        rospy.sleep(0.5)
    ...
    def move_angle(self, goal_angle):
        ...
        # 设置旋转角速度
        move_cmd.angular.z = goal_angular_speed#current_angular_speed
        ...
        while abs(turn_angle + angular_tolerance) < abs(goal_angle) and not rospy.is_shutdown():
            ...
            last_angle = self.rotation
            """
            if (abs(goal_angle - turn_angle) < radians(10)):
                current_angular_speed = abs(goal_angle - turn_angle) / radians(10) * goal_angular_speed
            ...
            #加速
            if (turn_angle < radians(15)):
                current_angular_speed = turn_angle / radians(15) * goal_angular_speed
                if current_angular_speed < min_angular_speed:
                    current_angular_speed = min_angular_speed
                if current_angular_speed > goal_angular_speed:
                    current_angular_speed = goal_angular_speed
