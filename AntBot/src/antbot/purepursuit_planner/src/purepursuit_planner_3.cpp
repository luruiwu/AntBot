...
public:
	...
	*/
	...
	void ROSSetup(){
		...
		/*ROS的消息*/
		ROS_INFO("%s::ROSSetup(): odom_topic = %s, command_topic_vel = %s, \
					position source = %s, desired_hz=%.1lf, min_lookahead = %.1lf, \
					max_lookahead = %.1lf, kr = %.2lf", 
  ...
  *///找到路径中的下一个点
  ...
  void SetRobotSpeed(double speed, double radius){
    ...
    ref_msg.angular.z = (radius == 0.0) ? 0 : AGVS_SPEED_K * speed / radius * 1.5;
