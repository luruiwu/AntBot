/*修改了purepursuit函数部分的路径跟踪的算法
 * 修改了速度命令发布函数SetRobotSpeed()
 * 2016.12.2 by Bob
 */
#include <string.h>//
#include <vector>//容器
...
//贝塞尔曲线的最小角度
#define MIN_ANGLE_BEZIER			0.261799388//15度
//贝塞尔曲线的控制点数
...
//距离最后一个位点的最小的距离
#define WAYPOINT_POP_DISTANCE_M		0.01
...
#define AGVS_FIRST_DECELERATION_MAXSPEED	0.20	// m/s
...
#define AGVS_SECOND_DECELERATION_MAXSPEED	0.10 	// m/s
	...
	ReturnValue AddWaypoint(Waypoint point){
		...
		pthread_mutex_lock(&mutexPath);
			if(vPoints.size() > 0){	//如果位点容器不为空
				...
				if( (aux.dX != point.dX) || (aux.dY != point.dY) )//判断要添加的位点和当前位点是否相同
...
/**************************判断角度大于等于最小的贝塞尔角度******************************************/
				if(fabs(dAngle) >= MIN_ANGLE_BEZIER){
					...
					// 如果角度大于等于45度，最大速度等级为MAX_SPEED_LVL1,否则为MAX_SPEED_LVL2
					if(fabs(dAngle) >= (Pi/4)){
...
private:
    ...
    geometry_msgs::Twist cmd_stop_msg;
...
/**************************机器人的追踪的路径追踪程序***********************/
	...
	//x,y,z速度
	cmd_vel.linear.x  = dAuxSpeed;
	cmd_vel.linear.y  = 0;//0.5 * (y1 / x1);
	cmd_vel.angular.z = 1.8 * (atan(y1 / x1) / 1.75);
    //设置机器人的速度和转向角度
    SetRobotSpeed(cmd_vel);
	...
	*/
	//设置机器人的速度
  void SetRobotSpeed(geometry_msgs::Twist cmd_vel){
	//发布速度命令
	...
	*/
	...
	int CalculateDirectionSpeed(Waypoint target_position){
		...
		double ux, uy, vx, vy;
		double beta = 0.0;
		static int last_direction = 0;//记录上一次的方向
		...
		/*向量法计算两个向量之间的夹角，机器人当前点的航向角和当前点与目标点之间的连线的夹角*/
		beta = acos( (ux * vx + uy * vy) / ( sqrt(ux*ux + uy*uy) * sqrt(vx*vx + vy*vy) ) );

		//如果夹角的绝对值小于90度
		if(fabs(beta) <= pi_medios){
			if(last_direction == 0){//如果是第一次，因为速度方向初始值为０
				ret = 1;
			}else{
				ret = 1;
				if( fabs(beta - pi_medios) <= max_diff){//如果相差小于等于５度
	...
	*/
    ...
    //从动作服务器获取新点
        ...
        if(goto_goal.target.size() > 1){	// 如果目标个数大于１，那么使用路径中的第二个点计算速度方向
        ...
        //当qPath中有路径时
          ...
          aux.BackWaypoint(&wLast);					//获取最后一个位点
					ROS_INFO("%s::MergePath: Adding new \
					        %d points from \
					        (%.2lf, %.2lf) to \
                  (%.2lf, %.2lf) and ",
                   sComponentName.c_str(),
