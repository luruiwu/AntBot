...
class PointPathManager(InteractiveMarkerServer):
        ...
        def __init__(self, name, frame_id, planner):
                ...
                # Send the path from the first point to the last one
                entry = self.menu_handler.insert( "Go", parent=h_second_entry, callback=self.startRouteCB)
                # Stops the current path
                entry = self.menu_handler.insert( "Stop", parent=h_second_entry, callback=self.stopRouteCB)
                # Sends the path from the last point to the first one
                entry = self.menu_handler.insert( "Go back", parent=h_second_entry, callback=self.reverseRouteCB) 
        ...
        def convertListOfPointPathIntoGoal(self, inverse = False):
                ...
                if inverse:
                        for i in reversed(self.list_of_points):
                                converted_list.append(goal(pose = Pose2D(i.pose.position.x, i.pose.position.y, 0.0), speed = i.speed)) 
                                # For now speed constant
                else:
                        for i in self.list_of_points:
                                #print 'convertListOfPointPathIntoGoal: %.2lf, %.2lf'%(i.pose.position.x, i.pose.position.y)
                                converted_list.append(goal(pose = Pose2D(i.pose.position.x, i.pose.position.y, 0.0), 
                                                      speed = i.speed)) # For now speed constant
