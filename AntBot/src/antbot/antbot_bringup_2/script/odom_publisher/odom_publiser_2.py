...
class PublishOdom():
	def __init__(self):
		...
		while not rospy.is_shutdown():
			...
			航迹推演
			"""
			#s1，s2代表单位时间内两对对称的轮子移动的距离,s为单位时间内实际移动的距离
			#单位时间内移动的轨迹假设为直线
			s1 = (encoder_dis[0] + encoder_dis[2])/2.0
			s2 = (encoder_dis[1] + encoder_dis[3])/2.0
			s = sqrt(s1 * s1 + s2 * s2)
			#print "s1 %f s2 %f s %f"%(s1, s2 ,s)
			
			#beta:机器人的速度方向与机器人坐标系的第一象限的X轴的夹角
			beta = radians(45) - atan2(s1, s2)#atan2(y, x)
			print ("beta : %f"%(beta*180/pi))
			
			s_x = copysign(cos(delta_th / 2.0) * s, s1)
			s_y = copysign(sin(delta_th / 2.0) * s, s2)
			
			#机器人坐标系统的变化
			r_x += s_x * cos(beta) - s_y * sin(beta)
			r_y += s_x * sin(beta) + s_y * cos(beta)
