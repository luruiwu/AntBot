...
class PublishOdom():
	def __init__(self):
		...
		while not rospy.is_shutdown():
			...
			#/*********************************************************************************/
			#机器人坐标系的变化
			r_x = (encoder_dis[0] + encoder_dis[2])/2.0 + (encoder_dis[1] + encoder_dis[3])/2.0
			r_y = (encoder_dis[0] + encoder_dis[2])/2.0 - (encoder_dis[1] + encoder_dis[3])/2.0
			
			#世界坐标系统的变化
			delta_x = r_x * cos(thPosition + delta_th/2) + r_y * cos(thPosition + pi/2 + delta_th/2)
			delta_y = r_x * sin(thPosition + delta_th/2) + r_y * sin(thPosition + pi/2 + delta_th/2)
