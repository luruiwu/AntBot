...
class Motor:
	def __init__(self):
		...
		self.encoder_msg.encoder = [0 for i in range(4)]
	...
	def loop(self):
		...
		while not rospy.is_shutdown():
			...
			self.encoder_msg.encoder[0] = self.Motor_Encoder[0]
			self.encoder_msg.encoder[1] = self.Motor_Encoder[1]
			self.encoder_msg.encoder[2] = self.Motor_Encoder[2]
			self.encoder_msg.encoder[3] = self.Motor_Encoder[3]
...
if __name__ == "__main__":
	try:
		...
		t1 = threading.Thread(target=motor.readSerial)
		t2 = threading.Thread(target=motor.loop)
