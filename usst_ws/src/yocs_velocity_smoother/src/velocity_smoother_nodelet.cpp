...
#include <ecl/threads/thread.hpp> // ecl？？？
...
void VelocitySmoother::reconfigCB(yocs_velocity_smoother::paramsConfig &config, uint32_t level)
{
  ...
  speed_lim_v  = config.speed_lim_v;//最大线性速度
  speed_lim_w  = config.speed_lim_w;//最大线性角速度
  accel_lim_v  = config.accel_lim_v;//限制线性加速度
  accel_lim_w  = config.accel_lim_w;//限制线性角加速度
  decel_factor = config.decel_factor;//限制减速因子
  decel_lim_v  = decel_factor*accel_lim_v;//能够减速多少
...
void VelocitySmoother::spin()
{
  ...
  while (! shutdown_req && ros::ok())
  {
    ...
    {
      // 速度输入一般情况下，最后一个速度是0, 但是如果不是的话，需要手动给定
