                                                                                             ...
                                                                                             ac("move_base", true)
{
   ...
   */
  //订阅odom
  odom_sub = n.subscribe<nav_msgs::Odometry>("/odom", 1, boost::bind(&Network::odomCallback, this, _1));
...
void Network::loop()
{
  ...
  while(ros::ok()){
    ...
    write_root["AGVState"]["x"] = odom_msg.pose.pose.position.x;
    write_root["AGVState"]["y"] = odom_msg.pose.pose.position.y;
...
void Network::odomCallback(const nav_msgs::Odometry::ConstPtr& msg)
{
  //从发布回调很慢，比用rostopic echo 慢，不知道为什么
  //不是开了线程的原因
...
void sigint_handler(int sig)
{
	 ...
	 * 保存机器人最后的位置姿态 
	 */
	pose_write_root["position_x"] = odom_msg.pose.pose.position.x;
	pose_write_root["position_y"] = odom_msg.pose.pose.position.y;
	pose_write_root["orientation_x"] = odom_msg.pose.pose.orientation.x;
	pose_write_root["orientation_y"] = odom_msg.pose.pose.orientation.y;
	pose_write_root["orientation_z"] = odom_msg.pose.pose.orientation.z;
	pose_write_root["orientation_w"] = odom_msg.pose.pose.orientation.w;
...
{
  ros::init(argc, argv, "yun_netwotk", ros::init_options::NoSigintHandler);
